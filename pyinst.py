...
def main():
    ...
    print(f'Building pragy-dl v{version} for {OS_NAME} {platform.machine()} with options {opts}')
    print('Remember to update the version using  "devscripts/update-version.py"')
    if not os.path.isfile('pragy_dl/extractor/lazy_extractors.py'):
    ...
    opts = [
        ...
        '--noconfirm',
        '--additional-hooks-dir=pragy_dl/__pyinstaller',
        *opts,
        'pragy_dl/__main__.py',
    ]
    ...
    SetVersion(exe, VSVersionInfo(
        ...
        kids=[
            StringFileInfo([StringTable('040904B0', [
                StringStruct('Comments', 'pragy-dl%s Command Line Interface' % suffix),
                StringStruct('CompanyName', 'https://github.com/pragy-dl'),
                StringStruct('FileDescription', 'pragy-dl%s' % (MACHINE and f' ({MACHINE})')),
                StringStruct('FileVersion', version),
                StringStruct('InternalName', f'pragy-dl{suffix}'),
                StringStruct('LegalCopyright', 'pukkandan.ytdlp@gmail.com | UNLICENSE'),
                StringStruct('OriginalFilename', f'pragy-dl{suffix}.exe'),
                StringStruct('ProductName', f'pragy-dl{suffix}'),
                StringStruct(
